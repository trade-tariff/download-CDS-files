...
class XmlFile(object):
    ...
    def get_footnote_types(self):
        ...
        if footnote_types:
            ...
            data = ('Action', 'Footnote type ID', 'Application code', 'Start date', 'End date', 'Description')
    ...
    def get_footnotes(self):
        ...
        if footnotes:
            ...
            data = ('Action', 'Combined', 'Footnote type ID', 'Footnote ID', 'Start date', 'End date', 'Description')
            worksheet.write_row('A1', data, g.excel.format_bold)
    ...
    def get_additional_codes(self):
        ...
        if additional_codes:
            ...
            data = ('Action', 'Additional code type', 'Additional code ID', 'Start date', 'End date', 'Description')
            worksheet.write_row('A1', data, g.excel.format_bold)
            worksheet.set_column(0, 0, 30)
            worksheet.set_column(1, 4, 20)
            worksheet.set_column(5, 5, 50)
            ...
            # Write markdown header
            self.md_file.new_header(level=1, title="Changes to additional codes")

            # Get data
            additional_codes = additional_codes.findall("AdditionalCode")
            for additional_code in additional_codes:
                row_count += 1
                AdditionalCode(self.md_file, additional_code, worksheet, row_count)
    ...
    def get_commodities(self):
        ...
        if commodities:
            ...
            # Write markdown header
            self.md_file.new_header(level=1, title="Changes to commodity codes")
            commodities = commodities.findall("GoodsNomenclature")
            for commodity in commodities:
                row_count += 1
                GoodsNomenclature(self.md_file, commodity, worksheet, row_count)
    ...
    def get_quota_order_numbers(self):
        ...
        if quotas:
            ...
            # Write markdown header
            self.md_file.new_header(level=1, title="Changes to quota order numbers")
    ...
    def get_quota_definitions(self):
        ...
        if quota_definitions:
            ...
            # Write markdown header
            self.md_file.new_header(level=1, title="Changes to quota definitions")
            ...
            for quota_definition in quota_definitions:
                row_count += 1
                quota_definition_object = QuotaDefinition(self.md_file, quota_definition, worksheet, row_count)
    ...
    def create_markdown_file(self):
        ...
        self.md_file = MdUtils(file_name=self.md_filename, title='CDS data extract')
