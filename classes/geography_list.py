...
class GeographyList(object):
    def __init__(self):
        ...
        for row in rows:
            geo = GeographicalArea(row[0], row[1], row[2], row[3], row[4], row[5], row[6])
            self.geography_list.append(geo)
            self.geography_dict[row[2]] = row[3]
        
class GeographicalArea(object):
    def __init__(self, geographical_area_sid, parent_geographical_area_group_sid, geographical_area_id, description, geographical_code, validity_start_date, validity_end_date):
